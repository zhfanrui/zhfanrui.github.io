...
<my:oi>PSEN</my:oi>（程序存储启用）是位于引脚 29 的输出信号，用来控制启用外部程序存储器。常与 EPROM（可擦可编写只读存储器）的 <my:oi>OE</my:oi>（输出使能）引脚连接。
...
<my:oi>EA</my:oi>（外部访问）是位于引脚 31 的输入信号。若接入高电平，则从内部 RAM, ROM 读取；反之接入低电平，则从外部读取。
